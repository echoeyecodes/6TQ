...
import {View, StyleSheet, Text, Dimensions, TextInput, TouchableNativeFeedback} from 'react-native';
...
const PlaceBetModal = ({visible,coinsLeft, onDismiss, onBetPlaced}) => {
  ...
  const placeBet = async () =>{
    ...
    if(regex>coinsLeft){
      alert("You do not have enough coins for this bet. Buy more coins or watch an ad!")
      return
    }
    setIsLoading(true)
    try{
      ...
      <Modal onDismiss={onDismiss} visible={visible}>
        <View style={[styles.container, {backgroundColor: theme.background}]}>
          <View style={styles.betContainer}>
            <Text style={[styles.subText,{color: theme.foreground}]}>Place your bet</Text>

            <TextInput value={value} keyboardType='numeric'
            onChangeText={(amount) => setValue(amount)}
            placeholder='Amount e.g 25' placeholderTextColor={theme.opacity} style={[styles.textInput, {color: theme.foreground, borderColor: theme.opacity}]} />

            <TouchableNativeFeedback onPress={() => !isLoading && placeBet()}>
              <View style={styles.doneBtn}>
                <Text style={[styles.doneBtnText, {
                  color: isLoading ? 'rgba(93, 17, 247, 0.5)' : 'rgba(93, 17, 247, 1)'
                }]}>Done</Text>
              </View>
